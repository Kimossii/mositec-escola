...
class DisciplinaAutorizacaoTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_admin_escola_tem_permissao_em_disciplina(): void
    {
        ...
        foreach (['ver', 'criar', 'editar'] as $acao) {
            $this->assertTrue(Gate::forUser($staff)->allows("disciplina.{$acao}"), "disciplina.{$acao}");
        }
    }

    public function test_professor_nao_tem_permissao_em_disciplina(): void
    {
        ...
        $this->assertFalse(Gate::forUser($professor)->allows('disciplina.ver'));
    }
}
