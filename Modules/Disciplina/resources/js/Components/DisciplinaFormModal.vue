...
watch(() => props.show, (show) => {
    if (!show) return;
    form.codigo = props.disciplina?.codigo ?? '';
    form.nome = props.disciplina?.nome ?? '';
    form.descricao = props.disciplina?.descricao ?? '';
    form.estado = props.disciplina?.estado ?? ESTADO.ATIVO;
});

function submeter() {
    const payload = { ...form };
    if (!props.disciplina) delete payload.estado;
    emit('submit', payload);
}
</script>
    ...
    <div v-if="show" class="modal d-block" style="background: rgba(0,0,0,0.5);" @click.self="emit('cancelar')">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content p-6">
                <h3 class="mb-5">{{ disciplina ? 'Editar Disciplina' : 'Nova Disciplina' }}</h3>
                <form @submit.prevent="submeter">
                    ...
                    <div v-if="disciplina" class="fv-row mb-7">
